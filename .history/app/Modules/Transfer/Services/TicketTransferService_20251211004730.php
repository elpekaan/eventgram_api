...
class TicketTransferService
{
    ...
    public function completeTransfer(TicketTransfer $transfer): void
    {
        DB::transaction(function () use ($transfer) {
            // 1. Transfer Statüsünü Güncelle
            $transfer->update([
            ...
            // 4. Satıcıya Ödeme Kaydı Oluştur (Payout)
            // Para hemen gitmez, admin onayına düşer (Güvenlik süresi)
            TransferPayout::create([
                'transfer_id' => $transfer->id,
                'seller_id' => $transfer->from_user_id,
                'amount' => $transfer->seller_receives,
                'status' => 'pending', // Admin ödeyecek
            ]);
        });
    }
}
